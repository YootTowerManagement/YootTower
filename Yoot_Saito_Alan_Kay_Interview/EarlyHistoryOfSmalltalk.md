> Source and credit: Markdown conversion of [Bret Victor](https://worrydream.com/)'s cleaned-up HTML edition of [Alan Kay](People.md#alan-kay-university-of-colorado-at-boulder-university-of-utah-xerox-parc-atari-apple-viewpoints-research-institute)'s HOPL paper, [The Early History Of Smalltalk](https://worrydream.com/EarlyHistoryOfSmalltalk/). The local Markdown and `EarlyHistoryOfSmalltalk-*` PNG files are derived from Bret's public [worrydream/EarlyHistoryOfSmalltalk](https://github.com/worrydream/EarlyHistoryOfSmalltalk) repository. Original publication/copyright notices from the source text are preserved below.

[original pdf](https://raw.githubusercontent.com/worrydream/EarlyHistoryOfSmalltalk/master/SmalltalkHistoryHOPL-scanned-2015-07-16.pdf) / [make corrections](https://github.com/worrydream/EarlyHistoryOfSmalltalk)
...
Most ideas come from previous ideas. The sixties, particularly in the [ARPA](Institutions.md#arpa-advanced-research-projects-agency)
...
handheld "[Dynabook](Technologies.md#dynabook)" in a way that would not be possible on a shared mainframe;
...
Early [Smalltalk](Technologies.md#smalltalk) was the first complete realization of these new points of view
...
—To Dave Evens, Bob Barton, Marvin Minsky, and [Seymour Papert](People.md#seymour-papert-mit-university-of-geneva-university-of-paris-national-physical-laboratory)

—To SKETCHPAD, [JOSS](Technologies.md#joss), LISP, and SIMULA, the 4 great programming conceptions of the sixties
...
with a [B5000](Technologies.md#b5000). I didn't have the perspective to really appreciate it at the time, but I did take note of its
...
![Moore's Law graph](EarlyHistoryOfSmalltalk-mooreslaw.png)
...
to debug the 6600, I read an article by [Gordon Moore](People.md#gordon-moore-uc-berkeley-cit-intel) which
...
Every newcomer got one. The title was "[Sketchpad](Technologies.md#sketchpad): A man-machine graphical communication 
...
![Sketchpad](EarlyHistoryOfSmalltalk-sketchpad.png)
...
the **when**s so that any change only had to compute through
...
and it was the first time I heard [Marvin Minsky](People.md#marvin-minsky-harvard-princeton-mit-ai-lab) speak. He put forth a terrific diatribe against
...
patterns and deep beauty "built-in."

![Pendery Paper Display Transducer Design](EarlyHistoryOfSmalltalk-penderypaperdisplay.png)
...
*promise* of its metastructures than its actual model. I spent
...
in particular *when* and in *what environment* did expressions get evaluated?

An elegant approach was suggested in a CMU thesis of [Dave Fisher](People.md#david-allen-fisher-cmu) [Fisher 70] on the synthesis of
...
![The Old Character Generator — early 1972](EarlyHistoryOfSmalltalk-oldcharacter.png)
...
![Children with Dynabooks from 'A Personal Computer For Children Of All Ages [Ka 72], Iconic Bubble Sort from 1972 LRG Plan [Ka 72a]](EarlyHistoryOfSmalltalk-childrenwithdynabooks.png)
...
Ingalls showed me the scheme working on the NOVA. He had coded it up (in BASIC!), added a lot of
...
5. The class holds the shared *behavior*
for its instances (in the form of objects in a program list)

6. To eval a program list, control is passed to
...
message. Thus ci <- de (with subscripting rendered as "○"
and multiplication as "*") means:

receiver | message c | ○ i <- d*e

The c is bound to the receiving object, and all of ○ i <- d*e is the message to it. The message is made
...
pairs are made from 2 element objects they can be indexed more simply: c hd, c tl, and c hd <- foo, etc.

"Simple" expressions like *a+b* and *3+4* seemed more troublesome at first. Did it really make sense
to think of them as:

receiver | message a | + b 3 | + 4

It seemed silly if only integers were considered, but there are many other metaphoric readings of
"+", such as:

!["kitty" |+ "kat" => "kittykat" [3 4 5 6 7 8] |+ 4 = [7 8 9 10 11 12]](EarlyHistoryOfSmalltalk-add.png)
...
[Larry Tesler](People.md#larry-tesler-stanford-university-xerox-parc-apple-inc-amazon-yahoo) (then working for POLOS) did not like the 
...
Synthesizer." A summer later, [Steve Saunders](People.md#steve-saunders-xerox-parc-hewlett-packard-atari-interval-research-corporation-apple-inc), another
of our bright summer students, was challenged to find
a way to accomplish John Chowning's very non-real-time
...
![Shazam iconic user interface, PYGMALION Iconic Programming, Simpula Hospital Simulation](EarlyHistoryOfSmalltalk-shazam.png)

[Chris Jeffers](People.md#chris-jeffers-xerox-parc) (who was a musician and educator, not
...
2-3 frames per second. Fast enough for the *phi*
...
professional animator [Eric Martin](People.md#eric-martin-xerox-parc) to visit and build SHAZAM
...
rendered more cleanly by scheduling individual
...
bother anyone as it was fairly easy to come up with *undo* and *replay* mechanisms to get around the
...
be the creation of interactive *tools* by the children.
...
like—they could come surprisingly close—they would be shown:
...
So, we decided we should teach *design*. And Adele came
...
are successful—and if a maximum effort of both children
and teachers was required to get the successes to happen.
Real pedagogy has to work in much less idealistic 
settings and be considerably more robust. Still, some 
successes are qualitatively different from no successes. We
...
![Circuit system by Steve Putz (age 15), Tangram designs are created by selecting shapes from a 'menu' displayed at the top of the screen. This system was implemented in Smalltalk by a fourteen year old girl [Kay 77]](EarlyHistoryOfSmalltalk-circuit.png)
...
The problem is not to get the kids to do stuff—they
love to do, even when they are not sure exactly what they
are doing. This correlates well with studies of early learning
of language, when much rehearsal is done regardless
of whether content is involved. Just *doing* seems to help.
What is difficult is to determine *what* ideas to put forth
and how *deeply* they should penetrate at a given child's
developmental level. This confusion still persists for
reading and writing of natural language—and for 
mathematics—despite centuries of experience. And it is the
...
in which it was first learned—and most metaphorical extensions 
...
world we must construct it. In other words, *we make not just to have, but to know.* But the having can
...
learned. The representations—whether markings, allusions, or physical controls—get in the way
...
The first is *fluency*, which in part is the process of building mental structures that disappear the
...
forth. If carried further one eventually becomes a kind of expert—but without deep knowledge in
other areas, attempts to generalize are usually too crisp and ill formed.

The second path is towards taking the knowledge as a *metaphor* than can illuminate other areas.
...
fluent and deep while building relationships with other fluent deep knowledge. Our society has 
...
situations that in contrast with other knowledge and other ways of thinking critically boost our
...
In other words, the *material* of a computer system is the computer itself, all of
the *content* and *function* is fashioned in software.

There are two important guidelines to be drawn from this:

- Material: If the design and development of the hardware computer material is done as carefully
...
- Content: Aside from the wonderful generality of being able to continuously shape new content
from the same material, *software* has three important characteristics:
...
Xerox must take these several points seriously if it is to survive and prosper in its new
business area of information media. If it does, the company has an excellent chance for several reasons:

- Xerox has the financial base to cover the large development costs of a small number of very
powerful computer-types and a large number of software functions.

- Xerox has the marketing base to sell these functions on a wide enough scale to garner back to
itself an incredible profit.
...
'good theatre' *aching to remember*." In other words, it is
the ability to evoke the audience's own intelligence 
and experiences that makes theatre work.

![Paul Rovner showing the iconic 'Lincoln Wand' ca. 1968](EarlyHistoryOfSmalltalk-paulrovner.png)

![The last Smalltalk-72 Interface, Ted Kaehler's iconic painting interface](EarlyHistoryOfSmalltalk-lastsmalltalk72.png)
...
all of the OS functions, files, printing and other [Ethernet](Technologies.md#ethernet)
...
![Two stylish ST-76 classes](EarlyHistoryOfSmalltalk-st76.png)
...
flash. In this case, it finds DocWindow method: **keyboard**,
...
not be solved easily by closed form mathematics—it
revealed a serious flaw in the disposition of workers given
the line's average probability of manufacturing defects.

![Alan Borning's Thinglab, a constraint-based iconic problem solver, Smalltalk-76 hierarchical class browser designed and built by Larry Tesler, The author's pen-based interface for ST-76](EarlyHistoryOfSmalltalk-thinglab.png)
...
locations of computations—this led to base/bounds registers, segment relocation, page MMUs, 
migratory processes, and so forth. Time-sharing was held back for years because it was "inefficient"—
...
the simplest scheme a single *and* gate) to trap if the operands aren't compatible with the ALU. Now all
...
see? One of the nicest late-binding schemes that is being experimented with is the *metaobject protocol*
...
some point it will be easier to have it carry even more information about itself—enough so its
...
a problem. Instead of a problem looking for methods, the methods look for problems—and Eurisko
looks for the methods of the methods. This has been called "opportunistic programming"—I think of
...
information about themselves in a form that permits *inferential* docking. Some of the ongoing work in
...
without the "new thing". Of course, such a theory is silly anyway—and yet, I think the enormous
...
*anything* whether good or bad, enough time was taken so that the result was usually good. Now we
...
worth—otherwise it is either too depressing or one stops too soon with trivial results.
...
![Byte Magazine, August 1981](EarlyHistoryOfSmalltalk-byte.png)
